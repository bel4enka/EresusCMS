...
class TAdminUI {
    ...
    $this->menu = array(
      array(
        ...
        "items" => array (
          ...
          array ("link" => "languages", "caption"  => admLanguages, "hint"  => admLanguagesHint, 'access'=>ADMIN, 'disabled'=>!MULTILANGUAGE),
  ...
  global $request, $locale;
    ...
    if ($locale['lang'] != DEFAULT_LANGUAGE) $result = '&lang='.$locale['lang'].$result;
  ...
  global $db, $request;
    ...
    if (count($items)) foreach($items as $item) {
      ...
      if (count($table['columns'])) foreach($table['columns'] as $column) {
        ...
        $result .= '<td'.(isset($column['align'])?' style="text-align: '.$column['align'].'"':'').(isset($column['wrap'])?($column['wrap']?'':' nowrap'):'').'>'.$value.'</td>';
      }
      $result .= "</tr>\n";
    }
    $result .= "</table>\n";
  ...
  global $request, $session, $plugins;
    ...
    if (!empty($session['message'])) {
      $result = InfoBox($session['message']).$result;
      $session['message'] = '';
    }
    /* OBSOLETE:END */
    ...
    if (!empty($session['errorMessage'])) {
      $result = ErrorBox($session['errorMessage']).$result;
