 ...
 *  Доступ           : только данный посетитель
 *************************************************************************************************/
 ...
 * **********************************
 *  Скорость доступа : высокая (возможно замедление в начале запроса)
 *  Время жизни      : сессия
 *  Доступ           : только данный посетитель
 **************************************************************************************************/

class EresusSessionCache extends EresusMemoryCache {
 /**
	* Конструктор
	*
	* @return EresusSessionCache
	*/
	function EresusSessionCache()
	{
		/*
		 * Возможно требуется проверка, активированы ли сессии?
		 */
		if (!isset($_SESSION['eresus_cache_storage'])) $_SESSION['eresus_cache_storage'] = array();
		$this->storage =& $_SESSION['eresus_cache_storage'];
		if (!isset($_SESSION['eresus_cache_index'])) $_SESSION['eresus_cache_index'] = array(
			'expires' => array(),
			'keys' => array(),
		);
		$this->storage =& $_SESSION['eresus_cache_index'];
		/*
		 * TODO:
		 *   Наверное лучше вынести очистку в конец обработки запроса, после того,
		 *   как вывод уже будет отправлен браузеру. Чтобы уменьшить задержки при
		 *   старте.
		 */
		$this->cleanup();
	}
