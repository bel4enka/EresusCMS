<?
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
# MyMacros (CMS Eresus™ Plugin)
# © 2006, ProCreat Systems
# Web: http://procreat.ru
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
class TMyMacros extends TListContentPlugin {
  var 
    $name = 'mymacros',
    $title = 'MyMacros',
    $type = 'client,admin',
    $version = '1.00b',
    $description = 'Создание собственных макросов',
    $settings = array(
    );
  var $table = array (
    'name' => 'mymacros',
    ...
    'sortDesc' => false,
    'columns' => array(
      array('name' => 'name', 'caption' => 'Макрос'),
      array('name' => 'value', 'caption' => 'Значение', 'maxlength' => 255),
    ),
    'controls' => array (
      'delete' => '',
      'edit' => '',
  ...
  global $page, $request;
    ...
    $form = array(
      ...
      'fields' => array (
        ...
        array ('type' => 'edit', 'name' => 'name', 'label' => 'Макрос $(', 'width' => '300px', 'maxlength' => '31', 'comment' => ')'),
  ...
  global $page;
    ...
    $page->addMenuItem('Расширения', array ("access"  => EDITOR, "link"  => $this->name, "caption"  => 'Макросы', "hint"  => "Управление собственными макросами"));
  }
  #--------------------------------------------------------------------------------------------------------------------------------------------------------------# 
}
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
