  ...
  global $db, $user, $page;
    ...
    if (count($items)) {
      foreach($items as $item) {
        ...
        if ($this->settings['stHasSubItems'] && !empty($item['submenu'])) $template = $this->settings['tmplSpecial'];
  ...
  global $page, $db;
    ...
    $form = array(
      ...
      'fields' => array (
        ...
        array('type'=>'text', 'value' => 'Использовать специальный шаблон если...'),
        array('type'=>'checkbox','name'=>'stHasSubItems','label'=>'есть подпункты'),
        array('type'=>'divider'),
