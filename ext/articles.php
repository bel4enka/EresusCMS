...
class TArticles extends TListContentPlugin {
  ...
  function update()
  {
    ...
    if (empty($item['preview']) || isset($request['arg']['updatePreview'])) $item['preview'] = $this->createPreview($item['text']);
  ...
  #--------------------------------------------------------------------------------------------------------------------------------------------------------------#
  function replaceMacros($template, $item, $dateFormat, $link='')
  {
    ...
    $result = str_replace(
      ...
      array(
        ...
        empty($item['preview'])?'':'<p>'.str_replace("\n", '</p><p>', StripSlashes($item['preview'])).'</p>',
