...
/**
 * Отправляет заголовок "text/plain"
 */
function plaintext()
{
	if (!headers_sent())
...
function callStack()
{
	...
	{
		if (strtolower($callstack[$i]['function']) != 'errorhandler')
		{
			...
			$result .= 'Call <b>' . (empty($callstack[$i]['class']) ? '' : $callstack[$i]['class']) .
				(empty($callstack[$i]['type']) ? '' : $callstack[$i]['type']) . $callstack[$i]['function'] .
				'</b>(' . $args . ')<br /><br />';
		}
	}
	$result .='</div>';
