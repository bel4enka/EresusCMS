...
class Eresus_Controller_Admin_Design extends Eresus_Controller_Admin_Abstract
{
	...
	public function sectionStylesInsert()
	{
		...
		$fp = fopen(Eresus_CMS::app()->getFsRoot() . '/style/'.arg('filename').'.css', 'w');
		fwrite($fp, $file);
		fclose($fp);
		HttpResponse::redirect(arg('submitURL'));
	}
	...
	public function sectionStylesDelete()
	{
		...
		$filename = Eresus_CMS::app()->getFsRoot() . '/style/'.arg('delete');
	...
	public function sectionStylesEdit()
	{
		...
		$item['html'] = trim(file_get_contents(Eresus_CMS::app()->getFsRoot() . '/style/'.$item['filename']));
	...
	public function sectionStylesList()
	{
		...
		$dir = Eresus_CMS::app()->getFsRoot() . '/style/';
		$hnd = opendir($dir);
