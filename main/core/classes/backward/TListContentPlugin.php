...
class TListContentPlugin extends TContentPlugin {
	...
	function up($id)
	{
		...
		if ($item['position'] > 0)
		{
			$temp = $Eresus->db->selectItem($this->table['name'],
				"$sql_prefix AND (`position`='".($item['position']-1)."')");
			$temp['position'] = $item['position'];
			$item['position']--;
			$Eresus->db->updateItem($this->table['name'],
				$item, "`".$this->table['key']."`='".$item['id']."'");
			$Eresus->db->updateItem($this->table['name'],
	...
	function down($id)
	{
		...
		if ($item['position'] < $count-1)
		{
			...
			$temp['position'] = $item['position'];
			$item['position']++;
			$Eresus->db->updateItem($this->table['name'], $item,
				"`".$this->table['key']."`='".$item['id']."'");
	...
	function clientRenderContent()
	{
		...
		else
		{
			...
			$this->itemsCount = $Eresus->db->count($this->table['name'],
				"(`section`='".$page->id."')".(in_array('active', $this->table['fields']) ?
					"AND(`active`='1')":''));
			if ($this->itemsCount)
			{
				$this->pagesCount = $this->settings['itemsPerPage'] ?
					((integer) ($this->itemsCount / $this->settings['itemsPerPage']) +
						(($this->itemsCount % $this->settings['itemsPerPage']) > 0)) :
					1;
			}
			if (!$page->subpage)
			{
	...
	function clientRenderList($options = null)
	{
		...
		$items = $Eresus->db->select(
			$this->table['name'],
			"(`section`='".$page->id."')".(strpos($this->table['sql'], '`active`') !== false ?
				"AND(`active`='1')":''),
