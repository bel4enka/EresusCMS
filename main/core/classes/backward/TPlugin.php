...
class TPlugin
{
	...
	function __item($item = null)
	{
		...
		$result['settings'] = $Eresus->db->escape(is_null($item) ? encodeOptions($this->settings) :
			$item['settings']);
	...
	*
	* @return  string  Метод возвращает строку, в которой заменены все макросы, совпадающие с полями
	*                  массива item
	*/
